    ...
    for primary_key, sub_dict in data.items():
        for sub_key, values in sub_dict.items():
            ...
            if prob == "meeting_scheduling":  # random:
                plt.yscale('symlog', linthresh=1)  # Linear threshold near zerotive, otherwise handle negatives
...
if __name__ == '__main__':
    is_clear = False
    scale = "query" #
    prob ="random"  #"meeting_scheduling" #"random"
    if is_clear:
        folder_to_save = "A_vars_cost_delta_algo_type_clear"
    else:
        folder_to_save = "A_vars_cost_delta_algo_type"

    graph_type = "A_CostVsQuerySizeAlgoType"
