...
def simply_avg_dict():
    ...
    for query_name,query_new_name in selected_query_types_and_new_name.items():
        ...
        for exp_name,exp_new_name in selected_explanations_and_new_name.items():
            ...
            data_to_put = avg_dict[selected_density][query_name]["bnb"][selected_vars_num][exp_name]
    ...
    for primary_key, sub_dict in data.items():
        for sub_key, values in sub_dict.items():
            ...
            plt.plot(x, y, linewidth=linewidth, linestyle=line_styles[primary_key], color=colors[sub_key],
                     label=f"{sub_key} ({primary_key})")  # Apply log if positive, otherwise handle negatives
            plt.plot(x, y, linewidth=linewidth, linestyle=line_styles[primary_key], color=colors[sub_key],
...
if __name__ == '__main__':
    ...
    if is_clear:
        folder_to_save = "E_constraint_count_delta_cost_expType_clear"
    else:
        folder_to_save = "E_constraint_count_delta_cost_expType"

    graph_type = "E_constraintCountDeltaCost"
